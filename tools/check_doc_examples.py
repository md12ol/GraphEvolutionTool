...
Bash and Rust blocks are deliberately out of scope. A shell block would have to
be executed to be checked, and most of them install things or clone repositories;
the Rust ones are mostly extension examples of types that do not exist yet, which
is what `data-example` marks.
"""
...
def main():
    failures = check_python() + check_toml()

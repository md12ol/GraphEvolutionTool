...
| **Step tables** | each `new-*.html`'s table matches its chain's markers, catching a page that stops one step short |
...
| **External links** | every outward link still resolves, with three retries, skipped rather than failed if the network is down entirely |
...
- **Prose, for redundancy.** Every page has been checked against the code, but nobody has read all
  ...
  overlap deliberately, since each is meant to be read alone, so keep those in step by hand.

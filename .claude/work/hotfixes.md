...
*The `#[allow(dead_code)]` on `GraphEvolver::python_fitness` entry that lived here since 2026-08-07
was removed 2026-08-11 — Michael, at the `/done run-dispatch` gate. #26 merged (PR #60, `97d9e02`,
2026-08-11T16:00:19Z); verified on `main`, not inferred: the attribute is gone from
`get/src/dispatch.rs:160` (the method moved there in the same PR's dispatch-extraction commit),
and `cargo clippy -p get --all-targets -- -D warnings` is clean without it. `Remove when:`
condition met.*

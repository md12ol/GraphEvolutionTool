...
# How wide an edge is drawn. Thin enough not to bury the nodes, thick enough
# that its colour is readable, which a hairline's is not.
EDGE_WIDTH = 1.8
...
def draw_convergence(examples, out_dir, directions):
    ...
    for panel, (config, members) in zip(flat, grouped):
        ...
        maximizing = True
        for example in members:
            maximizing = directions.get(example.name, True)
        keys = [
